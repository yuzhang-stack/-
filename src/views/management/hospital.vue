...
<!--        <el-input style="margin-right: 1%;" placeholder="CWID查询" class="search-input" v-model="userListData.userIdentifier"></el-input>-->
        <el-input style="margin-left: 1%;" placeholder="医院名称查询" class="search-input" v-model="userListData.hospitalName"></el-input>
        <!--        <el-button type="primary" slot="append" ></el-button>-->
        <el-button style="margin-left: 1%;" type="primary" @click="ChaxunTap">查询</el-button>
        <el-button style="margin-left: 1%;" type="primary" @click="addDialogFormVisible=true">添加医院</el-button>
...
<!--          <el-button size="mini" type="warning" plain @click="showEditDialog(scope.row,2)">分配角色</el-button>-->
    ...
    <!-- 编辑用户对话框 -->
    <el-dialog title="编辑医院" :visible.sync="editDialogFormVisible" @close='editDialogFormVisibleTap'>
      <el-form :model="addForm" label-width="80px" ref="addUserForm">
        <el-form-item label="医院名称" prop="CWID">
          <el-input v-model="addForm.hospitalName" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="所属地区">
          <el-select v-model="provinceName" filterable clearable placeholder="请选择所属省" @change="getRegion($event)">
            <el-option
              v-for="(item,index) in provinceList"
              :key="item.provinceId"
              :label="item.provinceName"
...
<script>
  ...
  export default {
    ...
    methods: {
      ChongzhiTap(){
        this.userListData.hospitalName = ''
        this.userListData.currentPage = 1
        this.initList();
      },
      // 省选择
